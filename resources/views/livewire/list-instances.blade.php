<div class="space-y-4">
    <flux:breadcrumbs>
        <flux:breadcrumbs.item>{{ __('Instances') }}</flux:breadcrumbs.item>
    </flux:breadcrumbs>

    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-4">
        @foreach($this->instances as $instance)
            <flux:callout :heading="$instance->display_name ?: $instance->name" :variant="$instance->getFluxVariant()">
                <!-- Type -->
                ...
                <x-slot name="controls">
                    <flux:button variant="ghost" size="sm" :href="route('instances.edit', $instance)" wire:navigate icon="pencil"/>
                </x-slot>
            </flux:callout>
        @endforeach
    </div>
